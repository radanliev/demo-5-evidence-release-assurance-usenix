...
| **Figure 1** (Architecture) and **Figure 2** (Witness protocol) | System model and trust boundaries; one credentialed session and the omission it defeats | TikZ in the manuscript source | `docs/usenix_paper_manuscript.pdf` | — |
| **Figure 3** (Scaling) | Merkle build to $N=10^6$ traces (1,815.13 ms mean of 5 on Apple M4 Max) | `scripts/run_release_benchmark.py` | `results/benchmark_summary.json`, `docs/figures/merkle_scaling.png` | ~25s |
| **Figure 4** (Throughput) | Peak 6,963 ± 89 ops/s at 4 workers (three-trace bundles) | `scripts/run_release_benchmark.py` | `results/benchmark_summary.json`, `docs/figures/parallel_throughput.png` | ~15s |
| **Figure 5** (Coverage matrix) | which of the 17 tamper vectors each executed baseline blocks (EviAssure 16/17 vs composed 10/17) and which of the 7 omission vectors each completeness baseline detects, cell by cell | `scripts/run_security_eval.py`, drawn by `scripts/generate_paper_pdf.py` (the Wilson-interval bar chart `comparative_block_rate.png` is still generated for the artifact) | `results/security_evaluation.json`, `docs/figures/coverage_matrix.png` | ~2s |
